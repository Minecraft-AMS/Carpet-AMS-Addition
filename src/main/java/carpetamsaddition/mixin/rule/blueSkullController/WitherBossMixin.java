...
public abstract class WitherBossMixin {
    ...
    private void shootSkullAt(int headIndex, LivingEntity target, CallbackInfo ci) {
        if (CarpetAMSAdditionSettings.blueSkullController == CarpetAMSAdditionSettings.blueSkullProbability.SURELY) {
    ...
    private Difficulty modifyDifficulty(ServerLevel world, Operation<Difficulty> original) {
        if (CarpetAMSAdditionSettings.blueSkullController == CarpetAMSAdditionSettings.blueSkullProbability.NEVER) {
